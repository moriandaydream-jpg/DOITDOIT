  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/posts" && request.method === "GET") {
        ...
        const posts = await listPosts(env, boardId, clientKey, admin);
        return cors({ posts }, origin);
      }
      ...
      if (path === "/api/comments" && request.method === "GET") {
        ...
        const comments = await listComments(env, postId, clientKey, admin);
        return cors({ comments }, origin);
      }
      ...
      return cors({ error: error.message || "proxy error" }, origin, error.status || 500);
    }
  },
};
...
function assertEnv(env) {
  ...
  if (!env.SUPABASE_SERVICE_ROLE_KEY) throw new Error("SUPABASE_SERVICE_ROLE_KEY secret이 필요해.");
...
function scrubPost(post, clientKey, admin) {
  ...
  return {
    ...safePost,
    can_edit: Boolean(admin || client_key === clientKey),
  };
}
