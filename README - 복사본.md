...
5. 연결 테스트 후 관리자 ID는 비워 둔 채 `공유 설정 저장`을 눌러 기본 설정을 저장합니다.
6. 메인 게시판에서 카카오 관리자 로그인을 합니다. 관리자가 비어 있으면 이 계정이 첫 관리자로 자동 등록됩니다.
...
카카오 로그인은 관리자 전용입니다. `owner_user_id`와 다른 계정은 관리자 권한을 받지 못하며, 화면에 로그인 User ID와 DB 관리자 ID를 표시합니다.
...
설치 페이지는 익명 사용자로 연결되므로 이미 관리자가 지정된 설정을 다시 저장할 수 없습니다. 기존 사이트의 설정 변경은 메인 게시판에서 카카오 관리자 로그인 후 관리 페이지를 사용합니다.
